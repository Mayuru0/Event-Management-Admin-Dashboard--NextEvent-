...
const OrganizerRejectedView = () => {
  const [selectedOrganizer, setSelectedOrganizer] = useState<User | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const organizersPerPage = 5;

  const { data, isLoading, isError } = useGetAllUsersQuery();
  const organizers: User[] = Array.isArray(data) ? data.filter(user => user.role === "organizer" && user.status === "rejected") : [];
...
export default OrganizerRejectedView;
