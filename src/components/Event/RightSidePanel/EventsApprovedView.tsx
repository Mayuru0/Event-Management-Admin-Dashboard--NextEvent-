...
const EventsApprovedView: React.FC<EventsApprovedViewProps> = ({ event, onClose }) => {
  ...
  const handleArchive = async () => {
    const result = await Swal.fire({
      title: "Archive Event?",
      text: `Archive "${event.title}"? It will no longer be visible to customers.`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#6b7280",
      cancelButtonColor: "#0065AD",
      confirmButtonText: "Yes, Archive",
    });
    if (!result.isConfirmed) return;
    try {
      await updateEventStatus({ eventId: event._id, status: "Archived" }).unwrap();
      Swal.fire({ title: "Archived!", icon: "success", confirmButtonText: "OK" });
      handleClose();
    } catch {
      Swal.fire({ title: "Error!", text: "Failed to archive event.", icon: "error" });
    }
  };
    ...
    <div className="absolute right-0 z-50 flex w-[600px] flex-col items-center gap-6 overflow-y-auto border-l bg-white p-6 shadow-[0_0_5px_0_rgba(0,0,0,0.25)]" style={{ maxHeight: "100vh" }}>
      ...
      <div className="flex w-[476px] flex-col gap-4 font-kulim text-[#1F1F1F]">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-semibold text-gray-500">Available Tickets</label>
      ...
      <div className="w-[476px] border-t pt-4">
        <div className="flex gap-3">
          <button
            ...
            className="rounded-md border border-red-500 px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 transition-colors"
          >
            🗑️ Delete
          </button>
          <button
            ...
            className="flex-1 rounded-md border border-gray-400 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors"
          >
            Archive
          </button>
          <button
